...
def recommend():
    ...
    try:
        ...
        mandi_price = mandi_trends.get(crop, 0)

        # 5. Link to user if mobile provided
        ...
        response = {
            ...
            'forecast_prices': forecasted_prices,
            'mandi_price': mandi_price,
            'explanation': f"{crop} is recommended due to favorable soil conditions and expected market price of ₹{forecasted_prices[0]}–₹{forecasted_prices[2]} over the next 3 months."
        }
...
def admin_test_market():
    ...
    url = cfg.get('agmarket_url') if isinstance(cfg, dict) else ''
    key = cfg.get('agmarket_key') if isinstance(cfg, dict) else ''
    if not url:
        return jsonify({'error':'agmarket_url not configured in server_config.json'}), 400

    params = {'location':'sample','crop':'Grapes'}
    headers = {}
    if key:
    ...
    try:
        ...
        try:
            ...
            payload = r.text
        return jsonify({'status_code': r.status_code, 'response': payload})
    except Exception as e:
        current_app.logger.exception('AGmarket test failed')
